...
const Home = (): JSX.Element => {
  ...
  const { data, loading, error, } = useQuery<ICharactersResponse>(FETCH_CHARACTERS);
  ...
  const renderCharacters = () => {
    ...
    return getCharacters()?.map((character: ICharacter | any, index: number) => {
      return <CharacterCard key={character?.id || character?.item?.id || index} character={character} />;
    });
  }
